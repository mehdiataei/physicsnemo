...
from physicsnemo.nn.functional.geometry.remeshing import Remeshing, remeshing
...
def test_remesh_namespace_is_separate_from_mesh_and_reuses_nn_functional():
    geometry = importlib.import_module("physicsnemo.geometry")
    remeshing_module = importlib.import_module("physicsnemo.geometry.remeshing")
    mesh_remeshing = importlib.import_module("physicsnemo.mesh.remeshing")
    functional = importlib.import_module("physicsnemo.nn.functional")
    functional_geometry = importlib.import_module("physicsnemo.nn.functional.geometry")

    assert geometry.remesh is remesh
    assert remeshing_module.remesh is remesh
    assert not hasattr(geometry, "Remeshing")
    assert not hasattr(remeshing_module, "remeshing")
    assert not hasattr(remeshing_module, "Remeshing")
    assert not hasattr(Mesh, "remesh")
    assert not hasattr(mesh_remeshing, "remesh")
    assert find_spec("physicsnemo.mesh.remeshing._remeshing") is None
    assert functional.remeshing is remeshing
    assert functional_geometry.Remeshing is Remeshing
