...
def test_cuda_mesh_adapter_matches_tensor_functional():
    source = sphere_icosahedral.load(subdivisions=2, device="cuda")
    adapted = remesh(source, 48)
    points, cells = remeshing(source.points, source.cells, 48)
    tensor_output = Mesh(points=points, cells=cells)

    torch.testing.assert_close(adapted.cells, tensor_output.cells)
    _assert_clean_topology(adapted)
    _assert_clean_topology(tensor_output)
