...
from physicsnemo.geometry import sobolev_deform
...
def test_sobolev_deform_geometry_namespaces_and_no_mesh_method():
    geometry = importlib.import_module("physicsnemo.geometry")
    deformation = importlib.import_module("physicsnemo.geometry.deformation")

    assert geometry.sobolev_deform is sobolev_deform
    assert deformation.sobolev_deform is sobolev_deform
    assert "sobolev_deform" in geometry.__all__
    assert "sobolev_deform" in deformation.__all__
    assert not hasattr(Mesh, "sobolev_deform")
    assert get_type_hints(sobolev_deform)["mesh"] is Mesh

    signature = inspect.signature(sobolev_deform)
