...
- Adds differentiable nearest-surface shrinkwrap through
  `shrinkwrap_points` and `physicsnemo.geometry.shrinkwrap`. Torch provides
  the reference search, NVIDIA Warp accelerates ``float32`` nearest-face
  queries on CPU and CUDA, and both backends replay the selected projection
  with PyTorch autograd. Triangulated documentation examples demonstrate
  weighted panel conformance and partial design-region projection onto a
  shape-optimization constraint.
